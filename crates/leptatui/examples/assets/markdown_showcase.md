...
Use the arrow keys or `j` and `k` to scroll one row, Page Up and Page Down to move five rows, `gg` to return to the top, and `G` to jump to the bottom. Tab and Shift+Tab focus links; Enter or Space opens the focused target. Local Markdown links and heading fragments stay in the reader. Shift+H and Shift+L move backward and forward through page history. Press `q` to quit.
